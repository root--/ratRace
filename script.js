...
function getWalls(){
	...
	// build the wall
		for(var j=0;j<wallLength;j++){
			...
			if(wallDir==0 && y!=0) y--; //up
			if(wallDir==1 && y<(ySize-1)) y++; //down
			if(wallDir==2 && x!=0) x--; //left
			if(wallDir==3 && x<(xSize-1)) x++; //right
		}
	}
}
...
function throwCheese(x,y){
  var objId=id(x,y);
	if(pt[objId].state!=="wall" ){
		if(chX>=0 && chY>=0) pt[id(chX,chY)].change("default");
  ...
  this.move=function(){
	...
	if(!moving){
	  for(var i=0;i<100;i++){
	    ...
	    if(moving=this.tryShift(randDir,0))break;// another direction
	  }
	}
  ...
  this.tryShift=function(direction,mark){
    ...
    if(this.view[direction]){
	  ...
	  if(direction==0 && ((this.steps-this.des[desId]>this.maxComeBack)||this.des[desId]==0)) { // direction and no max come back
	    this.shift(this.x,this.y-1); // shift rat
	    if(mark) this.des[desId]=this.steps; //increase decision counter
		move=true;
	  };
	  if(direction==1 && ((this.steps-this.des[desId]>this.maxComeBack)||this.des[desId]==0)) {
	    this.shift(this.x,this.y+1);
		if(mark) this.des[desId]=this.steps;
		move=true;
	  };
